...
for item in list(db.users.find()):
    print(item['name'])

# async def match(name):
#     print("Good")
#     await socketIO.emit("test", {"test": 12356, "name": name})
...
while True:
    ...
    for item in list(db.users.find()):
        me = face_recognition.load_image_file(item['file'])
        me_encoding = face_recognition.face_encodings(me)[0]
        known_face_names.append(item['name'])
        known_face_encodings.append(me_encoding)
